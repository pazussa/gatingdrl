...
* **<name>.ned** –  the physical topology as a `infrastructure` definition.
* **<name>.ini** –  simulation configuration that reproduces the exact
  cyclic traffic (period, payload, initial time_adjustment) *and* programmes the
...
only relies on public attributes of `core.infrastructure` objects, on the final
...
def _build_period_order(traffic_streams: list["Flow"]) -> dict[int, int]:
    """Mapa período→índice incremental (2000 µs⇒0 → UDP 2000, 4000 µs⇒1 → 2001…)."""
    unique = sorted({flow_generator.period for flow_generator in traffic_streams})
    return {period: iterator for iterator, period in enumerate(unique)}
...
def _first_tx_offset_us(schedule_res: "ScheduleRes", fl: "Flow") -> int:
    ...
    """
    first_link_id = fl.path[0]            # (source_node, destination_node) del primer salto
    for network_connection, ops in schedule_res.items():
        if network_connection.link_id != first_link_id:
            continue
        for flow_generator, operation_record in ops:
            if flow_generator.flow_id == fl.flow_id:   # ← comparación robusta
                return operation_record.start_time
...
def _ini_flows(
        traffic_streams: List["Flow"],
        schedule_res: ScheduleRes,
        infrastructure: "Network"           # ← necesitamos acceso al grafo
) -> str:
    """
    ...
    for flow_generator in traffic_streams:
        by_src.setdefault(flow_generator.src_id.lower(), []).append(flow_generator)
        by_dst.setdefault(flow_generator.dst_id.lower(), []).append(flow_generator)
    ...
    client_port_map: Dict[str, int] = {
        n: 2000 + iterator for iterator, n in enumerate(sorted(by_src.keys()))
    }
    ...
    for node, list_flows in by_src.items():
        ...
        for identifier, fl in enumerate(list_flows):
            ...
            txt += f"*.{node}.app[{identifier}].display-name   = \"{display_name}\"\n"

            # ➋ Offset = «Disponible (start time)» ⇒ operation_record.start_time
            ...
            txt += f"*.{node}.app[{identifier}].source.productionInterval = {fl.period*μs*1_000:.0f}ms\n"
            txt += f"*.{node}.app[{identifier}].source.initialProductionOffset = {off_ms}\n"
            txt += f"*.{node}.app[{identifier}].source.packetLength = {length}B\n"

            dport = client_port_map[node]           # puerto fijo por cliente
            txt  += f"*.{node}.app[{identifier}].io.destPort = {dport}\n"

            pcp = _pcp_for_flow(node, seen_per_client)
            txt += (f"*.{node}.app[{identifier}].bridging.streamIdentifier.identifier.mapping = "
                    f"[{{stream: \"{fl.flow_id}\", packetFilter: expr(udp.destPort == {dport})}}]\n")
            txt += (f"*.{node}.app[{identifier}].bridging.streamCoder.encoder.mapping = "
    ...
    sw_nodes = [n.lower() for n, d in infrastructure.network_structure.nodes(data=True)
    ...
    for node, list_flows in by_dst.items():
        ...
        for identifier, _ in enumerate(list_flows):
            txt += f"*.{node}.app[{identifier}].io.localPort = {2000+identifier}\n"
        txt += "\n"
...
def _ini_gcl(
    ...
    for network_connection, table in gcl.items():
        ...
        destination_node = network_connection.link_id[1] if isinstance(network_connection.link_id, tuple) else network_connection.link_id.split('-')[1]
        # Usa el mismo índice que figura en **.gateFilter
        port = gate_port

        # ────────────────  UNA sola cola "video" ─────────────────
        txt += f"*.{source_node.lower()}.eth[{port}].macLayer.queue.numTrafficClasses = 1\n"
        txt += f"*.{source_node.lower()}.eth[{port}].macLayer.queue.*[0].display-name = \"video\"\n"
        ...
        import math as _m
        hyperperiod = 1
        for _f in infrastructure.traffic_streams:
        ...
        durations = [(compact[(iterator+1)%len(compact)][0] - t) % hyperperiod
                     for iterator, (t, _) in enumerate(compact)]

        txt += f"*.{source_node.lower()}.eth[{port}].macLayer.queue.transmissionGate[0].time_adjustment = 0ms\n"
        txt += f"*.{source_node.lower()}.eth[{port}].macLayer.queue.transmissionGate[0].durations = "
        ...
        if recon_norm != orig_norm:
            ...
            logging.getLogger(__name__).debug(
                f"[GCL-CHECK] Diferencia detectada en {source_node}.eth[{port}] "
...
def write_ned(infrastructure: Network, outfile: os.PathLike, pkg: str, net_name: str) -> None:
    """Generate a .ned file representing *exactly* the passed infrastructure."""
    ...
    es_nodes = [n for n, d in infrastructure.network_structure.nodes(data=True) if d.get("node_type") == "ES"]
    sw_nodes = [n for n, d in infrastructure.network_structure.nodes(data=True) if d.get("node_type") == "SW"]
    ...
    for iterator, name in enumerate(client_nodes):
        submod_lines.append(_ned_node_line(name.lower(), "TsnDevice", 300, 200 + iterator * step))

    for iterator, name in enumerate(sw_nodes):
        submod_lines.append(_ned_node_line(name.lower(), "TsnSwitch", 500, 250 + iterator * step))

    for iterator, name in enumerate(server_nodes):
        submod_lines.append(_ned_node_line(name.lower(), "TsnDevice",
                                         700, 250 + iterator * step))

    # Connections – iterate original directed edges and create <--> pairs.
    conn_lines: List[str] = []
    added = set()
    for source_node, destination_node, edata in infrastructure.network_structure.edges(data=True):
        # Create deterministic id for unordered pair to avoid duplicates
        key = tuple(sorted((source_node, destination_node)))
        if key in added:
            continue
        added.add(key)
        ...
        def _alias(n: str) -> str:
            return n.lower()
        conn_lines.append(
            f"        {_alias(source_node)}.ethg++ <--> EthernetLink <--> {_alias(destination_node)}.ethg++;\n"
        )
        
    net_name = Path(outfile).stem          # p.ej.  «unidir»
    # Detectar bitrate (asumimos que todos los enlaces comparten valor)
    try:
        first_edge = next(iter(infrastructure.network_structure.edges(data=True)))
    ...
    ned = (
        ...
        f"infrastructure {net_name.lower()} extends TsnNetworkBase\n"  # Force lowercase here
        "{\n"
        "    parameters:\n"
...
def write_ini(
    ...
    """
    try:
        traffic_streams = list(infrastructure.traffic_streams)
        stream_count = len(traffic_streams)
        
        parts: List[str] = [_ini_general(net_name, gate_port)]
        try:
            parts.append(_ini_flows(traffic_streams, schedule_res, infrastructure))
        ...
        try:
            parts.append(_ini_gcl(gcl_tables, infrastructure, gate_port))
        except Exception as exc:
            import logging
...
def export_omnet_files(infrastructure: Network, schedule_res: ScheduleRes, gcl_tables: Dict[Link, List[Tuple[int, int]]], label: str, out_dir: os.PathLike, gate_port:int=2) -> Tuple[Path, Path]:
    ...
    write_ned(infrastructure, ned_path, pkg, net_name)
    write_ini(infrastructure, schedule_res, gcl_tables, ini_path, net_name, gate_port)
    ...
    # ──────────────────────────────────────────────────────────────
    complete_stream_count = len(infrastructure.traffic_streams)
    scheduled_ids = {
        flow_generator.flow_id
        for ops in schedule_res.values()
        for flow_generator, _ in ops
    }                                        # ← únicos
    ok_flows = len(scheduled_ids)

    error_message = f"Programados con éxito: {ok_flows}/{complete_stream_count} flujos"
    print(error_message)           # ← visible siempre
    logging.info(error_message)    # ← para quien tenga el event_recorder a nivel INFO
